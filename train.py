...
class MultiLingualCLIPDataset(torch.utils.data.Dataset):
    def __init__(self, image_dir, caption_file):
        ...
        self.langs = ['en'] + list(self.captions[0]["translated_captions"].keys())
    ...
    # Process the original caption under 'caption'
     original_caption = self.preprocess_text(cap_dict["caption"])
    # Process translated captions
     translated_caps = [self.preprocess_text(cap_dict["translated_captions"][lang]) for lang in self.langs if lang != 'en']
    
    # Assuming 'en' is in self.langs and represents the original English caption
     caps = [original_caption] + translated_caps
...
if __name__ == "__main__":
    ...
    model.to(device)
    train_dataset = MultiLingualCLIPDataset(os.path.join("data","images","downloaded_images"), os.path.join("translated_partial.json"))
    ...
    val_dataset = MultiLingualCLIPDataset(os.path.join("data", "images","downloaded_images"), os.path.join("translated_partial.json"))
