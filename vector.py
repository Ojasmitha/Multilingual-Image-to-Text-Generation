...
class MultiLingualCLIPDataset(torch.utils.data.Dataset):
    def __init__(self, image_dir, caption_file):
        ...
        # self.langs = ['caption'] + list(self.captions[0]["translated_captions"].keys())
        # self.langs = ['caption'] + list(self.captions[0]["translated_captions"].keys())
    ...
    # Adjusted to fetch the original caption and the translations
     caps = [self.preprocess_text(cap_dict["caption"] if lang == 'en' else cap_dict["translated_captions"][lang]) for lang in self.langs]
